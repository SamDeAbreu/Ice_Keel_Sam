...
keel_mask['g'] = sigmoid(z-versoria_keel(x, h, l, sigma, H)+1, a=2*epsilon)
...
strat['g'] = stratification(z, scale, z0, delta, b)*(1-keel_mask['g'])+28*keel_mask['g']
...
params = [Nx, Nz, wall, delta, epsilon, mu, eta, h, U, L, H, B, T, par, S, nu, scale, z0, Delta, b, strat, rho, rho0, prof1, prof2, l, keel_mask, accel]
param_names = ['Nx', 'Nz', 'wall', 'delta', 'epsilon', 'mu', 'eta', 'h', 'U', 'L', 'H', 'B', 'T', 'par', 'S', 'nu', 'scale', 'z0', 'Delta', 'b', 'strat', 'rho', 'rho0', 'prof1', 'prof2', 'l', "keel_mask", "accel"]
...
mixing.add_equation('dt(u) + dx(p) - nu*dz(q) = -w*q - (f/eta)*u - (wall*accel/eta)*(u-U)*0.1')
	#Eq 3d (Hester)
mixing.add_equation('dt(w) + dz(p) + nu*dx(q) = u*q - (f/eta)*w + par*B - (wall*accel/eta)*w*0.1')
	#Eq 3d (Hester)
mixing.add_equation('dt(C) - mu*(dx(dx(C)) + dz(dz(C))) = -(u*dx(C) + w*dz(C)) - mu*(dx(C)*dx(f)+dz(C)*dz(f))/(1-f+delta) - (f/eta)*(C-strat) - (wall*accel/eta)*(C-strat)*0.1')
	#Eq 3c (Hester)
...
if restart_sim:
	w['g'] = 0
	p['g'] = 0 #-1
	f['g'] = sigmoid(z-versoria_keel(x, h, l, sigma, H)-0.1, a=2*epsilon)
	...
	u['g'] = U * (1 - f['g'])
	C['g'] = stratification(z, scale, z0, Delta, b)*(1 - f['g']) + 28*f['g']
	ct['g'] = 0
	solver.stop_iteration = steps
	file_handler_mode = 'overwrite'
else:
	write, initial_timestep = solver.load_state('checkpoints-{0}/checkpoints-{0}_s3.h5'.format(sim_name))
